...
def _collect_assets(conn) -> list[dict]:
    ...
    # --- Discover schemas ---
    schemas_by_db = {db: [] for db in databases}
    try:
        # Optimization: use a single global query to fetch all schemas in the account
        cursor.execute("SHOW TERSE SCHEMAS IN ACCOUNT")

        col_names = [col[0].upper() for col in cursor.description]
        db_idx = col_names.index("DATABASE_NAME") if "DATABASE_NAME" in col_names else None
        ...
        if db_idx is not None:
            for row in all_schema_rows:
                db_name = row[db_idx]
                schema_name = row[name_idx]
                if db_name in schemas_by_db and schema_name not in _SKIP_SCHEMAS:
                    schemas_by_db[db_name].append(schema_name)
            use_fallback = False
        else:
            use_fallback = True
    except Exception as exc:
        print(f"  WARNING: could not list schemas globally, falling back to per-database: {exc}")
        use_fallback = True

    for db in databases:
        if use_fallback:
            # Fallback: discover schemas in each database if global query failed or lacks database_name
            try:
