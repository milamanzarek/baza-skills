...
async function handleIncomingMessage(message: IncomingMessage): Promise<void> {
  ...
  switch (content.type) {
    ...
    case 'button':
      await whatsapp.sendText(from, `Voce selecionou o botao: ${content.text || content.buttonId}`);
      break;

    case 'list':
      await whatsapp.sendText(from, `Voce escolheu o item: ${content.text || content.listId}`);
      break;

    case 'flow':
      await whatsapp.sendText(from, `Recebi seus dados do formulario (Flow).`);
      break;
    ...
    case 'location':
      await whatsapp.sendText(from, `Recebi sua localizacao.`);
      break;

    case 'reaction':
      await whatsapp.sendText(from, `Recebi sua reacao.`);
      break;

    default:
      await whatsapp.sendText(from, 'Desculpe, nao entendi o formato da mensagem. Como posso ajudar?');
      break;
  }
}
