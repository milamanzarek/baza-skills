...
> **BAZA LOCAL EDIT (guard header), added 2026-07-14 per the Skill Install Security Review**
> (`Baza: 00-POCHTA/AGENT_NOTES/DRAFT-Skill-Security-Review-Advisor-Orchestrator-Worker_Fable5_2026-07-14.md`,
> sanitization #1). Provisional pending Kamilla's Gate 3 ruling; only she may strike or amend it.
>
> 1. Workers default to the tool-less API fallback path (`references/fallbacks.md`). Do NOT
>    dispatch `agy --dangerously-skip-permissions` workers without Kamilla's explicit approval
>    in the current session. If a subtask needs tools and no such approval exists, ESCALATE.
> 2. Briefs never carry secrets, credentials, private logs, or unreviewed sensitive Baza or
>    client content. Brief content leaves the machine to the Google and Anthropic APIs.
> 3. State the run budget up front (the skill's own frame-step rule); never spend past it silently.
>
> Upstream capability preserved; nothing outside this block was modified. Source: repo HEAD
> `6ce858f` (2026-07-13) of github.com/Shubhamsaboo/awesome-llm-apps, manual copy, no installer.
